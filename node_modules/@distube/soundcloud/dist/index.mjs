...
var SoundCloudPlugin = class extends ExtractorPlugin {
  static async search(query, type = "track", limit = 10) {
    ...
    if (!SEARCH_SUPPORT.includes(type))
      throw new DisTubeError("INVALID_TYPE", SEARCH_SUPPORT, type, "type");
    ...
    if (type === "track") {
      ...
      return data2.collection.map((t) => new Song(new SoundCloudTrack(t)));
    }
  ...
  async resolve(url, options) {
    ...
    if (data.kind === "playlist") {
      ...
      if (!playlist.tracks?.length)
        throw new DisTubeError("SOUNDCLOUD_PLUGIN_EMPTY_PLAYLIST", "Playlist is empty.");
